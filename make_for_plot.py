...
no_clinic_col = find_col(df_raw, ["% of counties without a known clinic, 2020"], contains=["counties", "clinic", "2020"])
travel_col = find_col(df_raw, ["% of residents obtaining abortions who traveled out of state for care, 2020"],
                      contains=["traveled", "out", "state", "2020"])

# Rename for easier reference
rename_map = {}
if state_col: rename_map[state_col] = "state"
if no_clinic_col: rename_map[no_clinic_col] = "pct_counties_no_clinic_2020"
if travel_col: rename_map[travel_col] = "pct_traveled_outstate_2020"
...
# Scatter plot (with clean look)
scatter = plt.scatter(
    x, y, c=x, cmap='Reds', alpha=0.7, s=60,
    edgecolors='black', linewidth=0.5
)
plt.colorbar(scatter, label='% Counties Without Clinic →')

# Plot trend line (no legend label)
plt.plot(x_line, y_line, 'r--', linewidth=2)

# Annotate selected states
highlight_states = ["Mississippi", "Louisiana", "Kentucky", "Missouri", "Wyoming",
...
for s in highlight_states:
    ...
    if not row.empty:
        ...
        plt.annotate(
            s, (xv, yv), xytext=(8, 8), textcoords="offset points",
            fontsize=9, fontweight='bold',
            bbox=dict(boxstyle="round,pad=0.3", facecolor='lightyellow', alpha=0.7)
        )

# Labels and title
plt.xlabel("% of Counties Without an Abortion Clinic (2020)\n← More Accessible | More Restrictive →", fontsize=12)
plt.ylabel("% of Residents Traveling Out of State\nfor Abortion Care (2020)", fontsize=12)
plt.title("Limited Clinic Access Correlates with Higher Out-of-State Abortion Travel",
          fontsize=14, fontweight='bold', pad=20)

# Explanatory text box (keep this, no legend)
textstr = (
    f'• Positive correlation (r = {correlation:.3f})\n'
    f'• As clinic access decreases, out-of-state travel increases\n'
    f'• Restrictions displace rather than prevent abortions'
)
